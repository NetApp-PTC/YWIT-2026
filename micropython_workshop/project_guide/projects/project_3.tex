\chapter{Project 3: Safe Cracker}
...
Hold the module with the knob facing you and the pins pointing down: the silkscreen reads
...
\bbhole{encoder.header.clk}{A16}. Row A leaves the knob clear of the other wiring, so it is
easier to grip and turn. Then add these connections:
\begin{center}
\begin{tabular}{|l|l|l|}
    \hline
...
Insert the speaker's adapted Dupont leads into the bottom half of the breadboard:
the positive lead in \bbhole{speaker.lead.signal}{C20} and the negative lead in
\bbhole{speaker.lead.ground}{C22}. Then add these connections:
\begin{center}
\begin{tabular}{|l|l|l|}
    \hline
...
\begin{figure}[H]
    \centering
    \maybeincludegraphics[width=.95\linewidth]{project_3/wiring.png}
    \caption{Encoder and speaker wired to the XIAO ESP32-C3}
\end{figure}

\subsection{Understanding the Code}

Open the file named \texttt{project\_3\_safe\_cracker.py} in the IDE. Let's walk through
the key sections.
