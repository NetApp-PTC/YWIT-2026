...
Electronics is a fascinating world. This beginner's guide will introduce you to the essential
components and tools you'll need to start building your own simple circuits. We'll cover the
basics of breadboards, LEDs, light sensors (LDRs), buttons, resistors, and jumper cables. You should feel free to
...
\begin{figure}[H]
\centering
    \maybeincludegraphics[width=.8\linewidth]{electronics_preamble/breadboard_illustration.png}
\end{figure}
...
\begin{figure}[H]
\centering
    \maybeincludegraphics[width=.8\linewidth]{electronics_preamble/led_illustration.png}
\end{figure}
...
\begin{figure}[H]
\centering
    \maybeincludegraphics[width=.8\linewidth]{electronics_preamble/resistor_illustration.png}
\end{figure}

\section{LDRs: Sensing Light}
\subsection{What is an LDR?}
An LDR (light-dependent resistor), also called a photoresistor, is a resistor whose resistance
changes with light. More light typically means lower resistance; covering it with your hand means
higher resistance. Unlike an LED, an LDR has no polarity \- either orientation is fine.

\subsection{Using an LDR with a microcontroller}
A microcontroller pin measures voltage, not resistance directly. Pair the LDR with a fixed resistor
to make a voltage divider, then connect the middle of that divider to an analog input pin. As the
LDR's resistance changes, the voltage at the middle changes, and your program can read it as a number.
