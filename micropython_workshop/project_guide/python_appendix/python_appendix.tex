...
\begin{tabular}{|l|p{6.5cm}|l|}
    ...
    \hline
    \textbf{Dictionaries \& JSON} & Key-value data mapping (\texttt{\{"mode": "rainbow"\}}) for network APIs & Projects 5, 7 \\
    \hline
    \textbf{I\textsuperscript{2}C Communication} & Communicating with multiple digital sensors over a shared 2-wire bus & Projects 6, 7 \\
    \hline
    \textbf{Classes \& Objects} & Bundling data with the functions that act on it, so one screen or one device is described in one place & Project 7 \\
    \hline
    \textbf{Reading \& Writing Files} & Saving settings to the board's own filesystem so they survive a power cycle & Project 7 \\
    \hline
\end{tabular}
